...
public class Main {
        ...
        while (true) { // Keep the game running until the user decides to quit
            ...
            switch (choice) {
                ...
                case 4: // Multiplayer mode
                    ...
                    break;
                case 5:
                    mainInstance.playRankedMulti(scanner, players, new Random());
                    break;
                case 6:
                    mainInstance.changeDifficulty(scanner, players);
                    break;
    ...
    public static int displayMenu(Scanner scanner, List<Player> players, Bot bot) {
        while (true) {
            ...
            System.out.println("5 - Ranked Multiplayer Tournament");
    ...
    public void playMultiplayerMode(Scanner scanner, List<Player> players, Random random) {
        ...
        while (!guessed) {
            for (Player player : players) {
                determineLeader(players);
                String playerName = player.isLeader() ? "\u001B[31m" + player.getUsername() + "\u001B[0m" : player.getUsername(); // Leader privilege is a visual privilege - leader username displays in red
                System.out.println(playerName + "'s turn to guess!");
                ...
                if (scanner.hasNextInt()) {
                    ...
                    if (guess == randomNumber) {
                        ...
                        System.out.println("Correct! " + playerName + " guessed the number!");
        ...
        while (!validInput) {
            ...
            if (scanner.hasNextInt()) {
                ...
                if (choice == 1) {
                    ...
                    if (winner != null) {
                        String winnerName = "\u001B[32m" + winner.getUsername() + "\u001B[0m";
                        System.out.println(winnerName + ", you won! Please select the difficulty for the next game:");
    ...
    public void playRankedMulti(Scanner scanner, List<Player> players, Random random) {
        System.out.println("Welcome to Ranked Multiplayer Tournament!");
        System.out.println("Champions privilege : - 2 guess per round!");

        if (players.size() < 2) {
            System.out.println("At least 2 players are needed for a tournament!");
            return;
        }

        System.out.println("Select match format:");
        ...
        while (true) {
            if (scanner.hasNextInt()) {
                ...
                scanner.nextLine();
                if (format >= 1 && format <= 3) break;
            }
            System.out.println("Invalid choice, select 1, 2, or 3.");
        }

        int winsRequired = (format == 1) ? 1 : (format == 2) ? 2 : 3;
        ...
        while (tournamentPlayers.size() > 1) {
            ...
            for (int i = 0; i < tournamentPlayers.size(); i += 2) {
                if (i + 1 >= tournamentPlayers.size()) {
                    nextRoundPlayers.add(tournamentPlayers.get(i)); // Automatic advance
                    continue;
                }

                Player p1 = tournamentPlayers.get(i);
                Player p2 = tournamentPlayers.get(i + 1);
                System.out.println("Match: " + p1.getUsername() + " vs " + p2.getUsername());

                int p1Wins = 0, p2Wins = 0;
                while (p1Wins < winsRequired && p2Wins < winsRequired) {
                    Player winner = playMatch(scanner, p1, p2, random);
                    if (winner == p1) p1Wins++;
                    else p2Wins++;

                    System.out.println(p1.getUsername() + " [" + p1Wins + "] - [" + p2Wins + "] " + p2.getUsername());
                }

                Player matchWinner = (p1Wins > p2Wins) ? p1 : p2;
                System.out.println("\u001B[32mWinner: " + matchWinner.getUsername() + "\u001B[0m");
                nextRoundPlayers.add(matchWinner);
            }
        ...
        // Set the Champion
        Player champion = tournamentPlayers.get(0);
        System.out.println("\u001B[33m🏆 " + champion.getUsername() + " is the new Tournament Champion! 🏆\u001B[0m");

// Check if the current champion is in the tournament
        Player currentChampion = null;
        for (Player player : players) {
...
// Remove champion title from all losing champions
        for (Player player : players) {
            if (player.isChampion() && player != champion) {
                player.setChampion(false);
            }
        }

        champion.setChampion(true); // Assign new champion
        champion.saveToFile(); // Save the champion to file
    }

    private Player playMatch(Scanner scanner, Player p1, Player p2, Random random) {
        int maxRange = getMaxRange(p1.getDifficulty(), p1);
        int randomNumber = random.nextInt(maxRange + 1);
        ...
        while (true) {
            for (Player player : new Player[]{p1, p2}) {
                boolean isChampion = player.isChampion(); // Sprawdzamy, czy gracz jest mistrzem
                int attempts = isChampion ? 2 : 1; // Mistrz zgaduje dwa razy

                for (int i = 0; i < attempts; i++) {
                    String playerName = isChampion ? "\u001B[33m🏆 " + player.getUsername() + " 🏆\u001B[0m" : player.getUsername();
                    System.out.println(playerName + "'s turn! Enter your guess:");
                    ...
                    if (scanner.hasNextInt()) {
                        ...
                        if (guess == randomNumber) {
                            System.out.println("Correct! " + player.getUsername() + " wins this round!");
                            return player;
                        } else if (guess < randomNumber) {
                            System.out.println("TOO LOW!");
                        } else {
    ...
    public void changeDifficulty(Scanner scanner, List<Player> players) {
        System.out.println("Select new difficulty level:");
        System.out.println("1 - Easy (0-100)");
        System.out.println("2 - Normal (0-10,000)");
        System.out.println("3 - Hard (0-1,000,000)");
        System.out.println("4 - Custom (set your own range)");
            ...
            if (scanner.hasNextInt()) {
                ...
                switch (choice) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        setCustomDifficulty(scanner, players, null);
                        return;
                    default:
                ...
                for (Player player : players) {
                    player.setDifficulty(newDifficulty);
                }

                System.out.println("New difficulty is set to: " + newDifficulty);
                return;
            } else {
                System.out.println("Invalid input. Please enter a number.");
                scanner.nextLine(); // Clear invalid input
    ...
    public void determineLeader(List<Player> players) {
        Player leader = null;
        int maxScore = -1;

        for (Player player : players) {
            if (player.getMultiplayerScoreForDifficulty(player.getDifficulty()) > maxScore) {
                maxScore = player.getMultiplayerScoreForDifficulty(player.getDifficulty());
                leader = player;
            }
        }

        for (Player player : players) {
            player.setLeader(player == leader);
        }

        if (leader != null) {
            String leaderName = "\u001B[31m" + leader.getUsername() + "\u001B[0m";
            System.out.println("Current Leader: " + leaderName + " with score: " + maxScore);
        }
    }
